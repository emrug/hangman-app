// Dictionary below from Princetown - see copyright notice:
...
//split the dictionary by line breaks and store it in array
let dictionaryArr = dictionary.split('\n');

const wordList = [];

// loop through dictionary array and only return words with more than five letters and less than thirteen and
// no hyphens to use for selecting random words for hangman
dictionaryArr.forEach(word => {
    if(word.length > 5 && word.length < 13 && !word.includes("-") ) {
        wordList.push(word)
    }
})
